...
function forkRepo(user, reponame, branch, cb) {
  ...
  repo.fork(function(err) {
    onceReady(function() {
      repo.getRef(branch, function(err, commitSha) {
        // Create temp branch
        var refSpec = { "ref": "refs/heads/prose-integration", "sha": commitSha };
...
function patchFile(user, repo, branch, path, content, message, cb) {
  forkRepo(user, repo, branch, function(err, info) {
    saveFile(app.username, repo, "prose-integration", path, content, message, function(err) {
      if (err) return cb(err);
      var pull = {
        title: message,
        body: "This pull request has been generated by Prose.io.",
        base: app.state.branch,
        head: app.username + ":" + "prose-integration",
      };
      createPullRequest(app.state.user, app.state.repo, pull, function(err) {
        var forkedRepo = github().getRepo(app.username, repo);
        forkedRepo.deleteRef('heads/prose-integration', cb);
      });
    });
  });
}
